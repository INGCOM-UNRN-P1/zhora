---
title: "Guía de Extensión y Creación de Plugins: zhora"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de zhora"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-zhora-plugins)=
# Guía de Extensión y Plugins: zhora

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`zhora`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-zhora-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`zhora` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para zhora`.
- **Punto de Entrada Oficial**: `zhora.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-zhora-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para zhora."""
    name = "custom_rule"
...
Para que `zhora` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."zhora.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `zhora` la reconozca:
...
(manual-zhora-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `zhora` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = zhora.ejecutar_analisis(
...
(manual-zhora-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `zhora` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar zhora
        run: pip install -e ./zhora
        
      - name: Ejecutar Auditoría
        run: zhora check src/ include/ --json > reporte.json
````

---

(manual-zhora-plugins-ejercicios)=
